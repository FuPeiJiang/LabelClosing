...
const jsxSupported = Array(262)
jsxSupported[ts.SyntaxKind.ArrayLiteralExpression] = true
jsxSupported[ts.SyntaxKind.Block] = true
jsxSupported[ts.SyntaxKind.ObjectLiteralExpression] = true
jsxSupported[ts.SyntaxKind.ParenthesizedExpression] = true
jsxSupported[ts.SyntaxKind.ClassDeclaration] = true
jsxSupported[ts.SyntaxKind.JsxAttribute] = true
jsxSupported[ts.SyntaxKind.JsxElement] = true

const normalSupported = Array(235)
jsxSupported[ts.SyntaxKind.ArrayLiteralExpression] = true
jsxSupported[ts.SyntaxKind.Block] = true
jsxSupported[ts.SyntaxKind.ObjectLiteralExpression] = true
jsxSupported[ts.SyntaxKind.ParenthesizedExpression] = true
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let vscodeWeirdlyDecorationLength = 0
  ...
  function updateDecorations() {
    ...
    if (!languages.includes(language)) {
      ...
      function recurJSX(node: ts.Node) {
        //node types/kinds which are supported
        if (jsxSupported[node.kind]) {
      ...
      function recurNormal(node: ts.Node) {
        if (normalSupported[node.kind]) {
          var specialStart: number = -1
          let specialText: string = ""
          let useSpecialText: boolean = false
          let useCommentText: boolean = false
      ...
      let recur: typeof recurJSX|typeof recurNormal
      if (enableJSX) {
        recur = recurJSX
      } else {
        recur = recurNormal
    ...
    // activeEditor.setDecorations(closingLabelDecorationType, closingLabel.slice(0, 600)) //100 for 500 lines, 600 for 3000 lines

    if (closingLabel.length > vscodeWeirdlyDecorationLength) {
      foo()
    } else {
      activeEditor.setDecorations(closingLabelDecorationType, closingLabel) //100 for 500 lines, 600 for 3000 lines
    }
    function foo() {
      vscodeWeirdlyDecorationLength+=10;
      (activeEditor as vscode.TextEditor).setDecorations(closingLabelDecorationType, closingLabel.slice(0, vscodeWeirdlyDecorationLength))
      if (closingLabel.length > vscodeWeirdlyDecorationLength) {
        setTimeout(foo, 300)
      }
    }
    /* while (closingLabel.length > vscodeWeirdlyDecorationLength) {
      vscodeWeirdlyDecorationLength+=50
      // activeEditor.setDecorations(closingLabelDecorationType, closingLabel.slice(0, 2000)) //100 for 500 lines, 600 for 3000 lines
      activeEditor.setDecorations(closingLabelDecorationType, closingLabel.slice(0, vscodeWeirdlyDecorationLength)) //100 for 500 lines, 600 for 3000 lines
    } */
  }
}
